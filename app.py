...
with tab2:
   ...
   with tab2:
    ...
    # --- 2. Major Expressway Traffic (New Addition) ---
    st.divider()
    st.subheader("🛣️ Major Expressway Traffic (Real-Time)")
    
    # Specific Order: CTE, PIE, ECP, KJE
    expressways = [
        {"id": "CTE", "name": "Central Expressway", "color": "#1565C0"},
        {"name": "PIE", "id": "PIE", "full": "Pan Island Expressway", "color": "#2E7D32"},
    ...
    for i, ex in enumerate(expressways):
        ...
        # Adaptive background color for status tags
        status_bg = "rgba(40, 167, 69, 0.2)" if "Normal" in status else "rgba(220, 53, 69, 0.2)" if "Heavy" in status else "rgba(255, 193, 7, 0.2)"
        
        with ex_cols[i]:
            st.markdown(f"""
                <div style="background-color: var(--secondary-background-color); border-top: 4px solid {ex['color']}; padding: 12px; border-radius: 8px; border: 1px solid var(--border-color); height: 140px; color: var(--text-color);">
                    <div style="font-size: 0.9rem; font-weight: bold; color: {ex['color']};">{ex['id']}</div>
                    <div style="font-size: 0.7rem; opacity: 0.7; margin-bottom: 8px;">{ex.get('name', ex.get('full'))}</div>
                    <span style="background-color: {status_bg}; padding: 2px 6px; border-radius: 4px; font-size: 0.7rem; font-weight: bold;">{status}</span>
                    <div style="font-size: 0.75rem; margin-top: 10px; line-height: 1.2;">{detail}</div>
                </div>
            """, unsafe_allow_html=True)
    ...
    # --- 3. Network Status ---
    st.divider()
    st.subheader("🌐 Internet & Mobile Status")
    ...
    # --- 4. MRT Maintenance ---
    st.divider()
    st.subheader("🚆 Rail Engineering Advisory")
    ...
    for adv in advisories:
        st.markdown(f"""
            <div style="background-color: var(--secondary-background-color); border: 1px solid var(--border-color); padding: 12px; border-radius: 8px; margin-bottom: 10px; color: var(--text-color);">
                ...
                <div style="font-size: 0.85rem; margin-top: 8px; line-height: 1.4;">{adv['details']}</div>
            </div>
        """, unsafe_allow_html=True)
