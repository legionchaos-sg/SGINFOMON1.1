...
# THIS CREATES THE TAB VARIABLES - DO NOT MOVE THIS LOWER
...
with tab2:
    ...
    st.divider()
    st.subheader("🌐 Connectivity & Performance")
    ...
    with col_graph:
        ...
        for prov, score in zip(providers, uptime_scores):
            ...
            st.markdown(f"""<div style="margin-bottom:12px;"><div style="display:flex; justify-content:space-between; font-size:0.8rem; color: var(--text-color);"><span><b>{prov}</b></span><span>{score}%</span></div><div style="background-color: var(--border-color); border-radius: 4px; height: 10px; width: 100%;"><div style="background-color: {bar_color}; width: {score}%; height: 100%; border-radius: 4px;"></div></div></div>""", unsafe_allow_html=True)
        st.link_button("🚀 Run Instant Speed Test", "https://www.speedtest.net", use_container_width=True)

    with col_outage:
        st.write("**⚠️ Incident Log**")
        incidents = [("M1", "08:45", "Fiber latency in West."), ("Singtel", "14:20", "Brief DNS timeout."), ("Starhub", "Stable", "No issues."), ("Simba", "11:30", "Minor SMS delays.")]
        for p, t, m in incidents:
            status_color = "#28a745" if "Stable" in t or "Resolved" in m else "#ffc107"
            st.markdown(f"""<div style="font-size:0.8rem; border-left: 3px solid {status_color}; padding-left:8px; margin-bottom:12px; color: var(--text-color);"><b style="color: var(--primary-color);">{p}</b> <small style="opacity:0.6;">{t}</small><br>{m}</div>""", unsafe_allow_html=True)

    # --- 3. MRT Maintenance ---
    st.divider()
    st.subheader("🚆 Maintenance Advisory")
    advisories = [
        {"line": "Circle Line (CCL)", "impact": "Single Platform Service", "details": "Ongoing tunnel strengthening between Mountbatten and Paya Lebar.", "status": "In Progress"},
