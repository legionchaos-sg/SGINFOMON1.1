...
# SG INFO MONITOR - Weather & Traffic Update 10.9.4 (Wide Mode Patch)
...
# 2. Adaptive CSS - Enhanced for Wide Layout & Height Fix
st.markdown("""
    <style>
    /* Force the main container to use the full width and prevent clipping */
    .main .block-container { max-width: 98%; padding-left: 2rem; padding-right: 2rem; color: var(--text-color); }
    
    /* Ensure tabs expand vertically to fit all panels */
...
with tab1:
    with st.container(): # Height fix for Tab 1
        # 1. Clocks
        t_cols = st.columns(6)
...
with tab2:
    with st.container(): # Height fix for Tab 2
        ...
        # --- 5. Weather (Original Logic Maintained) ---
    ...
    st.caption("Data source: LTA / SMRT / SBS / NEA Open Data. Wide Mode Enabled.")
