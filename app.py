...
with tab4:
    ...
    # 1. CORE DATA & TREND LOGIC
    # Current snapshot from Tab 1 data points
    current_coe = {
    ...
    # 2. SELECTION ROW (Aligned to gold 10 layout)
    p_c1, p_c2, p_c3 = st.columns([1.5, 1, 1.5], vertical_alignment="center")
    
    with p_c1:
        v_cat = st.selectbox("Select Prediction Category:", list(current_coe.keys()), key="g10_t4_cat_p")
    
    with p_c2:
        m_bias = st.select_slider("Market Sentiment:", options=["Bearish", "Neutral", "Bullish"], value="Neutral", key="g10_t4_bias_p")
    
    with p_c3:
        # Calculate Bid-to-Quota Ratio
        b_q_ratio = current_coe[v_cat]['b'] / current_coe[v_cat]['q']
        st.markdown(f"""
            <div style="background: rgba(255,255,255,0.05); padding: 5px; border-radius: 5px; border: 1px solid #333; text-align:center;">
                <small>Bid-to-Quota Ratio</small><br><strong>{b_q_ratio:.2f}x Demand</strong>
            </div>
        """, unsafe_allow_html=True)
    ...
    # 3. PREDICTION LOGIC
    # Base = Current Price * Demand Ratio Weight * Sentiment Multiplier
    sentiment_mult = {"Bearish": 0.97, "Neutral": 1.0, "Bullish": 1.04}[m_bias]
    # Prediction uses a 60% weight on Bid/Quota pressure and 40% on historical trend
    pred_val = current_coe[v_cat]['p'] * (1 + (b_q_ratio - 1.4) * 0.05) * sentiment_mult * current_coe[v_cat]['trend']
    
    diff = pred_val - current_coe[v_cat]['p']
    diff_pct = (diff / current_coe[v_cat]['p']) * 100

    # 4. PRIMARY OUTPUTS
    res_l, res_r = st.columns([2, 1])
    
    with res_l:
        st.markdown(f"#### Upcoming COE Prediction: {v_cat}")
        st.markdown(f"""
            <div style="background: rgba(0,255,127,0.08); padding: 25px; border-radius: 12px; border: 1px solid #00ff7f; text-align: center;">
                <span style="font-size: 0.85rem; color: #888; text-transform: uppercase;">Estimated Next Tender Price</span><br>
                <span style="font-size: 2.8rem; font-weight: bold; color: #00ff7f;">${pred_val:,.0f}</span><br>
                <span style="font-size: 1.1rem; color: {'#00ff7f' if diff >= 0 else '#ff4b4b'};">
                    {'▲' if diff >=0 else '▼'} ${abs(diff):,.0f} ({diff_pct:+.2f}%)
                </span>
            </div>
        """, unsafe_allow_html=True)

    with res_r:
        st.metric("Allocated Quota", f"{current_coe[v_cat]['q']:,}")
        st.metric("Submitted Bids", f"{current_coe[v_cat]['b']:,}")
        st.progress(current_coe[v_cat]['q'] / current_coe[v_cat]['b'], text="Fulfillment Prob.")

    # 5. HISTORICAL SIMULATION CHART
    st.markdown("---")
    st.write(f"**{v_cat} Price Trajectory Simulation**")
    # Generating a dummy 6-round trend ending in the prediction
    hist_points = [current_coe[v_cat]['p'] * (1 - (i*0.015)) for i in range(5, 0, -1)]
    hist_points.append(current_coe[v_cat]['p'])
    hist_points.append(pred_val)
    
    chart_df = pd.DataFrame({"Price ($)": hist_points})
    st.area_chart(chart_df, height=220, color="#00ff7f")

    if st.button("📡 Finalize Prediction Metadata", use_container_width=True, key="g10_t4_save_p"):
        st.success(f"Prediction for {v_cat} logged with {m_bias} sentiment bias.")
